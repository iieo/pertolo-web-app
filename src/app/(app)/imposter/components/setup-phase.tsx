...
import { PersonIcon, GearIcon, PlayIcon, CrossCircledIcon, PlusIcon, GridIcon, ShuffleIcon } from '@radix-ui/react-icons'
    ...
    <div className="min-h-[100dvh] w-full bg-black flex flex-col p-4 sm:p-6 md:max-w-3xl md:mx-auto">
      <div className="flex-1 flex flex-col space-y-8">
        <div className="text-center pt-4">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-2 tracking-tight">Imposter</h1>
          <p className="text-base sm:text-lg text-[#888] font-medium">Find the secret agents</p>
        </div>

        {error && (
          <Alert variant="destructive" className="border-2 border-[#fb8500] bg-[#ffb703]/30 animate-pulse rounded-xl">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            {/* Category Selection */}
            <div>
              <Label className="text-sm font-semibold text-[#888] mb-3 block uppercase tracking-wider px-1">Word Category</Label>
              
              <div className="bg-[#111] border border-[#222] rounded-2xl overflow-hidden">
                <div className="p-6 text-center border-b border-[#222] bg-gradient-to-b from-[#1a1a1a] to-[#111]">
                  <p className="text-[#888] text-xs font-bold uppercase tracking-widest mb-2">Current Selection</p>
                  <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight mb-1">
                    {gameState.selectedCategoryId ? (selectedCategory ? selectedCategory.name : 'Loading...') : 'Random Category'}
                  </h3>
                ...
                <div className="flex divide-x divide-[#222]">
                  ...
                  >
                    <ShuffleIcon className={`w-4 h-4 ${!gameState.selectedCategoryId ? 'text-[#fb8500]' : 'text-[#666]'}`} />
                    Random
                  </button>

                  <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger asChild>
                      <button className={`flex-1 py-4 flex items-center justify-center gap-2 font-bold transition-all active:bg-[#222] ${gameState.selectedCategoryId ? 'bg-[#fb8500]/10 text-[#fb8500]' : 'bg-transparent text-white hover:bg-[#1a1a1a]'}`}>
                        <GridIcon className={`w-4 h-4 ${gameState.selectedCategoryId ? 'text-[#fb8500]' : 'text-[#666]'}`} />
                        Select
                      </button>
                    </DialogTrigger>
                    <DialogContent className="bg-[#111] border-[#222] text-white max-h-[80vh] overflow-y-auto w-[95%] max-w-2xl rounded-2xl">
                      <DialogHeader>
                        <DialogTitle className="text-2xl font-bold text-white">Select Category</DialogTitle>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 py-4">
                        {categories.map((category) => (
                          ...
                          >
                            <h3 className={`font-bold text-base leading-tight mb-1 ${gameState.selectedCategoryId === category.id ? 'text-[#fb8500]' : 'text-white'}`}>
                              {category.name}
                            </h3>
            ...
            <div className="bg-[#111] rounded-2xl border border-[#222] p-4 space-y-6">
              <div>
                <Label className="text-sm font-semibold text-[#888] mb-3 block uppercase tracking-wider">Imposters Count</Label>
               ...
               <div className="flex items-center justify-between pt-2">
                 <div className="space-y-0.5">
                    <Label className="text-white font-bold text-base">Show Category to Imposter</Label>
                    <p className="text-[#666] text-sm">Makes it easier for imposters to blend in.</p>
                 </div>
                 <Switch
                    checked={gameState.showCategoryToImposter ?? false}
                    onCheckedChange={(checked) => setGameState(prev => ({ ...prev, showCategoryToImposter: checked }))}
